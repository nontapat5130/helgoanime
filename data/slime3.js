...
var pages ={
	home: Barba.BaseView.extend({
		...
		onEnter: function() {
			...
			if (!is_season_flg){
				...
				$(".pop-season2").show();
				setTimeout(function () {
					$(".pop-season2").addClass("show");
				}, 500)
				$(".pop-season2-close,.pop-season2-cover").on({ click: this.season_pop_end })
			}
		},
		season_pop_end: function(){
			$(".pop-season2").removeClass("show").fadeOut(500);
		},
		...
		onEnterCompleted: function () {
			$(".term-item").on({
				click: function (e){
					...
					$(this).addClass("active");
					$(".movie-list").attr("data-category", $(this).attr("data-category"));
					$(".bd").attr("data-show",$(this).attr("data-category"))
				}
			});
			$(".bd-page-pagelink-link,.bd-footer a").on({
				click: function(e){
					e.preventDefault();
					var diff = (window.innerWidth < 900) ? 80 : 0;
					$("html,body").animate({ scrollTop: $($(this).attr("href")).offset().top - diff }, 300);
					$(".bd-footer").removeClass("open");
				}
			});
			$(".bd-image-main").on({
				click:function(e){
					...
					$(".bd-popup-inner h2").empty().append('<img src="' + $(this).find("img").attr("data-title") + '">');
					$(".bd-popup-inner .image").empty().append('<img src="'+src+'">');
					$(".bd-popup").addClass("open");
				}
			});
			$(".bd-popup-close,.bd-popup-inclose").on({
				click:function(){
					$(".bd-popup").removeClass("open");
				}
			});
			$(".bd-image-thumb img").on({
				click:function(e){
					e.preventDefault();
					$("#title-" + $(this).attr("data-id")).attr("src", $(this).attr("data-title"));
					$("#main-" + $(this).attr("data-id")).attr("src", $(this).attr("src"));
					$("#main-" + $(this).attr("data-id")).attr("data-title", $(this).attr("data-title"));
					//var diff = (window.innerWidth<900) ? 80:0;
					//$("html,body").animate({ scrollTop: $("#title-" + $(this).attr("data-id")).offset().top - diff }, 300);
				}
			});
			var flg = false;
			$(".bd-footer-spmenu").on({
	...
	teaser: Barba.BaseView.extend({
		namespace: 'teaser',
		onEnterCompleted: function(){
			window.teaser.onEnter();
		},
		onLeaveCompleted: function() {
			window.teaser.onLeave();
		}
	})
}
pages.home.init();
...
/* ใขใใกใผใทใงใณใฎ้–ขๆ•ฐใ’่จ่ฟฐ
--------------------------------------------------*/
...
/* ใขใใกใผใทใงใณใฎ้–ขๆ•ฐใ’ๅฎ่ก
--------------------------------------------------*/
Barba.Pjax.getTransition = function () {
	return PageTransition;
};
/* headใฟใฐๅ…ใฎๆธใๆใ
--------------------------------------------------*/
...
function character_setting(){
	...
	//็งปๅ•ใขใใกใผใทใงใณ
	function move(i){
		status.is_animation = true;
		//่กจ็คบๅใๆฟใ
		setView();
		//ใขใใกใผใทใงใณ
...
function keyword_setting(){
	...
	//ใใผใธๅ…ใขใณใซใผๅฆ็
	$('a[href^="#"]').on({
		click: function (e) {
			e.preventDefault();
			var t = $($(this).attr("href")).offset().top;
			//่กจ็คบใฟใ–ใ้•ใๅ ดๅใฎๅฆ็
			if (t == 0) {
